# MIDNIGHT REPO INTELLIGENT --- EXECUTION 01

## Foundation, Boundaries & Project Intelligence Contracts
...
Create the canonical foundation for `Midnight Repo Intelligent` as an
extension of Midnight Performance without copying the independent Repo
project wholesale and without violating Performance's current contract
that it does not own repository truth.

### Required outcomes

1.  Define typed, versioned identities/contracts for:
    ...
    -   ResearchQuestion
    -   EvidenceBundle
    -   KnowledgeClaim / ProjectInsight
    -   GraphLink
    -   Exposure
    -   LearningOutcome
    -   CostRecord
2.  Explicitly distinguish `observed`, `derived`, `inferred`,
    `statistical`, `predicted`, and `recommended`.
3.  Define source classes: live repository, Performance evidence, Memory
    reference, Git/VCS metadata, GitHub external repository, official
    docs, standards, papers, web.
4.  Define trust/freshness/provenance fields and deterministic identity
    rules.
5.  Define project authorization so one project cannot retrieve another
    project's private intelligence.
6.  Create ports/interfaces for repository intelligence, Performance
    reads, Memory bridge, external discovery, fetch/parse, embeddings,
    model generation, graph projection, clock, budget meter.
7.  Keep core usable with optional providers unavailable.
8.  Add architecture/contract tests proving no direct sibling DB access
    and no duplicate durable-memory authority.

### Deep questions

-   What is the minimum derived state Repo Intelligent must own?
-   Which existing Performance identities can be referenced instead of
    duplicated?
-   How can a source-code symbol be referenced stably across rename/move
    without pretending identity is perfect?
-   Which fields are safe to send externally and which must remain
    local?
-   What must be rebuildable versus durable?
-   How will later Executions add graph and research without changing
    foundational identity semantics?

### Verification

Prove deterministic serialization/round-trip, schema-version rejection,
cross-project denial, unknown-field fail-closed behavior where required,
stable IDs, and clean operation with no model/network configured.
