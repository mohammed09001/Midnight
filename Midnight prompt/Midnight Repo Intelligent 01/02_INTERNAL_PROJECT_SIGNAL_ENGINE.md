# MIDNIGHT REPO INTELLIGENT --- EXECUTION 02

## Internal Project Signal & Hotspot Intelligence Engine
...
Turn the current project plus Midnight Performance history into
**signals of learning need** without exposing raw stored prompts/code as
the product output.

### Required outcomes

1.  Incrementally resolve repository entities: repository →
    package/module → file → symbol/code region → test/config/doc where
    supported.
2.  Join those typed repository references to Performance evidence:
    prompt runs, agent runs, change sets, verification, feedback,
    outcomes, episodes, similarity/relationship graph references.
3.  Read qualified Memory context only through the existing
    bridge/contract.
4.  Build hotspot signals from evidence such as:
    -   repeated edits/rework;
    -   repeated failed or flaky verification;
    -   recurring prompt intent around the same component;
    -   high change frequency/churn;
    -   repeated rollback/supersession;
    -   cross-component coupling;
    -   unresolved evidence gaps;
    -   unfamiliar/new dependency or subsystem;
    -   long-running repeated task patterns.
5.  Do not equate high churn with bad code. Produce an explainable
    `need_score` with evidence and uncertainty.
6.  Add decay/time-window logic so old activity does not dominate
    forever.
7.  Generate abstract `ResearchQuestion` candidates from signals while
    stripping private identifiers unless policy allows them.
8.  Deduplicate semantically equivalent questions before any external
    call.

### Required scoring

Implement a configurable, explainable score such as:
`need = activity × friction × recurrence × impact × learning_gap × freshness`
with every factor inspectable and replaceable. Missing evidence lowers
confidence rather than being fabricated.

### Cost constraints

All hotspot detection should be deterministic/local by default. Model
use is optional and only for ambiguous intent/topic abstraction after
deterministic extraction fails.

### Verification

Use synthetic histories that distinguish: - healthy active development
from repeated failure; - one large refactor from chronic rework; - a
recently hot component from an old historical hotspot; - two projects
with identical filenames but isolated evidence.
