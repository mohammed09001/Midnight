# MIDNIGHT REPO INTELLIGENT --- EXECUTION 03

## Project Knowledge Graph & Cross-Graph Linking Engine
...
Create a project-specific knowledge graph that links internal project
structure, Performance history, qualified Memory references, and later
external knowledge without turning the graph into a second source of
truth.

### Graph model

Required node families: - Project / Repository / Snapshot - Package /
Module / File / Symbol / CodeRegion - PromptRun / AgentRun / ChangeSet /
Verification / Episode / Outcome - MemoryRecordRef - Topic / Concept /
Technology / Dependency / Pattern / FailureMode - ResearchQuestion -
ExternalRepository / ExternalDocument / Paper / Standard / Article -
Evidence / Claim / Insight - Exposure / UserFeedback / LearningOutcome

Required edge families include: `CONTAINS`, `DEPENDS_ON`,
`CALLS_OR_REFERENCES`, `CHANGED_IN`, `DISCUSSED_IN`, `VERIFIED_BY`,
`FAILED_IN`, `RELATED_TO`, `SIMILAR_TO`, `CONTRADICTS`, `SUPERSEDES`,
`SUPPORTED_BY`, `DERIVED_FROM`, `ABOUT`, `RELEVANT_TO`,
`EXTERNAL_ANALOGUE_OF`, `LEARNED_FROM`, `EXPOSED_AS`.

Every non-structural edge must carry provenance, method/version,
confidence where meaningful, timestamp/freshness, and uncertainty.

### Required architecture

-   Extend/reuse Performance's existing rebuildable relationship-graph
    concepts where compatible; do not mutate its canonical semantics
    merely to fit this product.
-   Introduce a Repo Intelligent graph projection/overlay when ownership
    differs.
-   Use typed external references for Memory and external sources.
-   Support deterministic rebuild from authoritative inputs.
-   Support incremental update of affected subgraphs.
-   Support local neighborhood traversal, bounded multi-hop traversal,
    community/topic grouping, and path explanation.
-   Graph traversal must be budget-aware and cycle-safe.
-   Separate exact structural edges from probabilistic semantic edges.

### Cross-graph strategy

Design a **federated graph bridge**, not a physical mega-graph
requirement: - Performance graph keeps Performance lineage. - Memory
remains externally referenced. - Project code graph owns derived
structural topology. - External research graph owns captured external
knowledge. - A project-intelligence overlay links them through stable
typed references.

### Verification

Prove rebuild determinism, no orphan cross-project edges,
confidence/provenance preservation, cycle safety, incremental update
correctness, and path explanations that cite underlying evidence.
