# MIDNIGHT REPO INTELLIGENT --- EXECUTION 04

## External Discovery, GitHub & Web Relevance Engine
...
Search the external world only when it can enrich the current project,
directly or indirectly, and rank discoveries by project value rather
than generic popularity.

### Sources

Implement adapters behind capability interfaces for: - GitHub repository
search/metadata and selected repository evidence; - official
project/library documentation; - standards/specifications; - scholarly
search where useful; - explicitly allowed web search/fetch; - optional
feeds/video metadata only if product policy enables them.

The independent Repo/Curiosity adapters may inspire contracts and tests
but must be audited and repaired/reimplemented as needed; do not inherit
defects blindly.

### Discovery modes

1.  **Direct relevance**: same dependency, framework, error class,
    architecture, algorithm, protocol, repository pattern.
2.  **Structural analogue**: another project solves a similar
    architectural relationship even with different technology.
3.  **Adjacent learning**: a concept that explains a hotspot or repeated
    user activity.
4.  **Counterexample**: evidence that challenges the current approach.
5.  **Freshness watch**: meaningful upstream/spec/security/API change
    affecting an active project component.

### Query formation

Research questions originate from internal signals. External queries
must minimize leakage: - abstract private symbol/repository names by
default; - include public dependency names only when policy allows; -
never send raw private prompts/diffs automatically; - record the
abstraction transform and query provenance.

### Relevance ranking

Rank using explainable features:
`project_match + hotspot_match + evidence_quality + source_authority + freshness + novelty + learning_value + diversity - redundancy - privacy_risk - cost`

Do not use stars/popularity as a proxy for correctness.

### Retrieval strategy

Use coarse-to-fine retrieval: - cheap metadata/search first; - fetch
only top candidates; - parse only necessary sections; - deeper
repository/document analysis only after relevance gate; - stop when
marginal information gain falls below threshold.

### Verification

Fixture-test rate limits, malformed payloads, stale sources, duplicate
URLs/repos, poisoned README instructions, inaccessible pages,
contradictory sources, and zero-result behavior. Live smoke tests are
optional and never the only release evidence.
