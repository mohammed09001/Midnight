# MIDNIGHT REPO INTELLIGENT --- EXECUTION 05

## Evidence Synthesis & Project Knowledge Engine
...
Convert internal + external evidence into useful project knowledge while
preserving source truth and never presenting generated prose as
evidence.

### Required pipeline

`EvidenceBundle → claim candidates → evidence alignment → contradiction check → verification/qualification → synthesis → ProjectInsight`

A ProjectInsight must contain: - concise statement; - why it matters to
this project now; - internal evidence references; - external evidence
references when used; - affected project entities/topics; - claim
kind; - confidence/uncertainty; - freshness; - actionable learning
direction (not automatic code modification); - counterevidence/gaps; -
cost provenance for its generation.

### Rules

-   Never copy raw prompts or large code blocks into the insight unless
    the user explicitly requests source evidence and policy permits it.
-   Prefer abstraction: "authentication retry path has repeated
    verification friction" over replaying private prompt text.
-   Distinguish fact, interpretation, hypothesis, recommendation, and
    open question.
-   Contradictory evidence is surfaced, not averaged away.
-   One strong authoritative source can outweigh many weak sources, but
    the reason must be explicit.
-   Generated synthesis must be re-grounded against evidence IDs before
    exposure.
-   If evidence is insufficient, produce a question/gap rather than a
    confident insight.

### Model strategy

Use deterministic extraction for metadata/structure; small models for
bounded classification; strong models only for cross-source synthesis
requiring reasoning. Cache synthesis by evidence-set hash +
prompt/template version + model family.

### Verification

Create adversarial tests for unsupported synthesis, source
contradiction, stale evidence, hallucinated citations, prompt injection
in source text, evidence removed after cache creation, and model output
that references nonexistent evidence IDs.
