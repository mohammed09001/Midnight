# MIDNIGHT REPO INTELLIGENT --- EXECUTION 06

## Intelligent Learning & Terminal Experience Engine
...
Expose Repo Intelligent as a calm, high-signal learning layer in the
terminal while Midnight Performance continues observing in the
background.

### Experience principles

The terminal must not become a noisy feed. The engine earns
interruption.

Support user-invoked surfaces such as: - project briefing / "what should
I understand now?"; - explain a hotspot; - show useful external
discoveries; - connect an internal project pattern to an external
concept; - show why an insight was selected; - save/dismiss/ask
deeper; - pause/resume proactive enrichment; - inspect
source/provenance; - budget/status/diagnostics.

### Proactive exposure gate

An insight may be surfaced proactively only when: - relevance passes
threshold; - evidence quality passes threshold; - novelty is
sufficient; - it is not recently exposed; - current terminal context is
not in a protected high-focus operation; - budget allows; - privacy
policy allows; - the expected learning value exceeds interruption cost.

### Presentation

Default card/brief format:
`WHAT → WHY NOW → PROJECT CONNECTION → EXTERNAL CONNECTION (optional) → CONFIDENCE/GAPS → NEXT LEARNING ACTION`

Never dump internal prompt history by default. Never claim "you are
doing this wrong" from activity alone. Allow the user to drill down to
evidence.

### Feedback loop

Record
`shown / opened / saved / dismissed / asked_deeper / useful / not_useful`,
but do not overfit immediately to one interaction. Use bounded decay and
minimum evidence before preference changes.

### Performance

Rendering must be local and fast. Expensive research/synthesis occurs
asynchronously before exposure and never inside the display tick.

### Verification

Test no-network startup, slow background research, terminal interruption
policy, deduped cards, stale-card invalidation, safe text rendering, and
