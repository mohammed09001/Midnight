# MIDNIGHT REPO INTELLIGENT --- EXECUTION 07

## Continuous Learning, Trigger & Curiosity Loop Engine
...
Build a continuous but bounded learning loop that evolves with the
project and user activity without becoming an uncontrolled autonomous
crawler.

### Trigger classes

-   repository change event;
-   completed Performance episode;
-   repeated hotspot threshold crossing;
-   verification failure cluster;
-   new dependency/technology;
-   Memory contradiction/new promoted lesson;
-   upstream freshness TTL expiry;
-   explicit user request;
-   scheduled low-priority maintenance window.

### Loop

`capture trigger → coalesce → compute changed scope → refresh internal signals → score knowledge gap → check existing graph/cache → decide whether external research is justified → create bounded job → discover → verify → synthesize → graph-link → rank → queue exposure → collect outcome`

### Required controls

-   event coalescing/debounce;
-   per-component cooldown;
-   duplicate research-question suppression;
-   maximum traversal depth;
-   maximum follow-up questions;
-   maximum remote sources;
-   maximum model calls/tokens;
-   maximum wall time;
-   cancellation;
...
-   explicit "why did this run?" audit trail.

### Novel termination logic

Implement a measurable **marginal knowledge gain** stop rule. Continue
research only when a new step is expected to add evidence diversity,
contradiction resolution, freshness, or a missing graph connection. Stop
on redundancy saturation.

Do not optimize for number of discovered items. Optimize for useful,
evidence-backed learning per unit of attention and cost.

### Verification

Simulate bursty file changes, repeated identical prompts, offline
periods, provider rate limits, cancellation/restart, duplicate triggers,
and a research branch whose marginal gain becomes zero.
