# MIDNIGHT REPO INTELLIGENT --- EXECUTION 08

## Cost--Quality Router, Caching & Budget Engine
...
Achieve the highest defensible intelligence quality at the lowest
necessary cost by routing each task to the cheapest method capable of
satisfying its quality contract.

### Required router

Classify work into: - deterministic; - retrieval-only; -
classification/ranking; - semantic matching; - bounded synthesis; - deep
cross-source reasoning.

For every job compute: `required_quality`, `uncertainty`,
`freshness_need`, `privacy_risk`, `expected_information_gain`,
`estimated_cost`, `latency_class`.

### Escalation policy

Start cheap. Escalate only when a measurable quality gap remains.
Examples: - AST/import graph before LLM repository explanation; -
FTS/graph neighbors before embeddings; - cached external metadata before
new fetch; - small relevance model before strong synthesis model; -
targeted community traversal before global graph summarization.

Evaluate research patterns demonstrated by GraphRAG dynamic community
selection and LazyGraphRAG: early relevance pruning can preserve quality
while avoiding processing irrelevant graph communities; lazy/on-demand
summarization can avoid large up-front indexing cost. Adapt the
principles to code/project intelligence rather than copying an
implementation.

### Caches

Implement distinct caches for: - content-addressed source bytes/text; -
parsed/chunked documents; - embeddings; - search results; - relevance
decisions; - graph community/topic summaries where used; - synthesis
outputs keyed by evidence-set hash and template/model version.

Every cache needs invalidation/freshness semantics and privacy scope.

### Budget policy

Support per-job and per-project: - requests; - input/output tokens; -
remote bytes; - wall time; - strong-model calls; - search/fetch calls.

Budget exhaustion must degrade transparently, never silently substitute
weak unsupported knowledge.

### Quality gates

Cheap output is accepted only if it passes evidence coverage and
uncertainty thresholds. Otherwise escalate or return an explicit gap.

### Verification

Use a fake priced provider to prove: - cache hits avoid duplicate
spend; - irrelevant graph branches are pruned; - strong model is not
called for deterministic work; - quality gate forces escalation when
needed; - hard budget prevents runaway cost; - final cost ledger
reconciles exactly with executed calls.
