# MIDNIGHT REPO INTELLIGENT --- EXECUTION 09

## Privacy, Trust, Provenance & Research Security Engine
...
Make project intelligence safe for private repositories and hostile
external content.

### Threat model

Cover: - secrets in code/prompts/transcripts; - private repository names
and internal identifiers leaking through search queries; - prompt
injection in README/issues/docs/web pages; - malicious source
instructing the model to ignore system rules or run commands; - poisoned
repositories; - fabricated citations; - dependency confusion / lookalike
repositories; - cross-project cache leakage; - stale authorization; -
oversized/decompression/parser abuse; - external content trying to
become Memory truth.

### Required controls

-   local redaction before outbound boundaries;
-   query abstraction/minimization;
-   source allow/deny policy and trust classes;
-   domain/repository canonicalization;
-   bounded fetch size/type/time;
-   content hashes;
-   untrusted-content delimiters and instruction isolation;
-   model prompts that explicitly treat source text as data;
-   evidence-ID validation after generation;
-   project-scoped cache keys;
-   export disabled unless policy allows;
-   no shell execution from discovered content;
-   no automatic dependency installation;
-   no direct promotion of external claims to durable Memory without
    qualification and explicit Memory contract.

### Provenance

Every exposed insight must be able to answer: - where did this come
from? - when was it captured? - which internal/external evidence
supports it? - which transformation/model/version produced it? - what
was redacted or unavailable? - what contradicts it?

### Verification

Create a malicious fixture corpus with prompt injection, fake GitHub
repo, embedded token, PII, giant page, cross-project cache collision,
and unsupported model citation. All must fail safely while legitimate
