# MIDNIGHT REPO INTELLIGENT --- EXECUTION 11

## Federated Graph Retrieval, Local↔Global Reasoning & Knowledge Paths
...
Provide one retrieval interface over the federated project-intelligence
graphs while keeping canonical stores separate.

### Query classes

Route questions into: - entity-local: one file/symbol/component; -
activity-local: what happened around a hotspot; - project-global:
themes, repeated friction, architecture evolution; - external-analogue:
how related projects/concepts approach this; - learning-path: what
should the user understand next; - provenance/path: why does the system
believe an insight.

### Retrieval plan

Use hybrid evidence: 1. exact IDs/typed references; 2. lexical/FTS; 3.
structural graph neighbors; 4. temporal Performance relationships; 5.
semantic retrieval when needed; 6. bounded community/topic traversal; 7.
external graph only when query intent/knowledge gap requires it.

Adopt a DRIFT-like principle: a local query may expand through
high-value community context and generate bounded follow-up
subquestions, but every expansion must pass information-gain and cost
gates.

### Path explanations

Return compact paths such as:
`hot file → repeated ChangeSets → failed verification cluster → concept topic → external repository pattern → verified insight`.

Paths must distinguish exact edges from probabilistic edges and display
uncertainty.

### Global summaries

Do not precompute expensive summaries for every community by default.
Prefer lazy summaries cached only when a query or repeated hotspot
justifies them.

### Verification

Test local/global routing, bounded traversal, cycle handling, community
pruning, deterministic tie-breaking, graph updates after file rename,
missing Memory, no external network, and explanations with complete
evidence lineage.
