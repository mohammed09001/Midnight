# MIDNIGHT REPO INTELLIGENT --- EXECUTION 12

## End-to-End Integration, Repair & Release Qualification
...
Integrate Executions 01--11 into one project-scoped Midnight Repo
Intelligent capability and prove it from a clean environment. This is a
release gate, not a feature-expansion execution.

### End-to-end scenario that must work

1.  A user initializes Midnight on a project.
2.  Midnight Performance records normal work in the background.
3.  Repository changes and Performance episodes create bounded internal
    signals.
4.  A repeated-friction hotspot crosses a learning-need threshold.
5.  Repo Intelligent retrieves internal project context and qualified
    Memory references.
6.  It determines whether existing internal knowledge is sufficient.
7.  If not, it forms a privacy-minimized research question.
8.  External discovery finds a small set of relevant
    sources/repositories.
9.  Sources are fetched/parsed/verified under trust and cost policy.
10. Evidence is linked into the federated graph.
11. A grounded ProjectInsight is synthesized.
12. The terminal exposes it only if relevance/novelty/interruption gates
    pass.
13. User feedback is recorded.
14. Later Performance outcomes can be associated with the exposure
    without claiming causality.
15. Re-running the same pipeline is idempotent and uses caches rather
    than repeating spend.

### Integration audit

Search for and remove/reconcile: - duplicate contracts; - duplicate
graph owners; - direct Memory DB access; - direct Performance ledger
bypass; - provider-specific imports in core; - dead experimental
paths; - raw source text accidentally treated as knowledge; -
network/model calls in rendering paths; - missing budget enforcement; -
cross-project cache/state leakage.

### Clean qualification

Run from a clean environment with fixture providers first. Then, if
configured, run one bounded live external-source smoke test.

Required evidence: - full unit/integration suite; - end-to-end fixture
scenario; - offline scenario; - malicious-source scenario; -
restart/replay scenario; - budget-exhaustion scenario; - stale-cache
invalidation; - graph rebuild determinism; - privacy
query-minimization; - provider-neutral harness test; - cost
reconciliation; - final repository-wide regression suite.

### Final review questions

-   Does this genuinely enrich the user rather than generate noise?
-   Can every important insight explain "why now"?
-   Can the system work usefully with no external model/network?
-   Does external research occur because of a demonstrated knowledge
    gap?
-   Is quality protected by evidence gates rather than model prestige?
-   Can expensive work be skipped without lowering correctness?
-   Is the graph a transparent reasoning aid rather than an opaque truth
    machine?
-   Can a different coding agent use the same capability without
    architecture changes?
-   Can an independent reviewer reproduce the final claims?

Only mark the parent goal `YES` if all blocking evidence passes after
the final edit.
