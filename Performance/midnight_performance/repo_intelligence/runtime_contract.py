...
CANONICAL_STAGE_INVENTORY: tuple[StageInventoryEntry, ...] = (
    ...
    StageInventoryEntry(RuntimeStage.ROUTE_CHEAPEST_QUALIFIED_RESOLVER, "repo_intelligence_pipeline deterministic/local routing gate", "repo_intelligence_pipeline.run_pipeline", "job/cost records", "StageOutcome", "runtime tests", "repo_intelligence.cost_quality.route remains library-only until a production PricedExecutor exists", StageOwnershipStatus.CANONICAL, "Current production routing is deterministic/local-first; adaptive learned routing is later work."),
    ...
    StageInventoryEntry(RuntimeStage.ATTENTION_RANK, "repo_intelligence.terminal_learning.decide_terminal_card", "repo_intelligence_pipeline.run_pipeline", "Exposure history only after selection", "StageOutcome", "terminal + runtime tests", "repo_intelligence.attention remains library-only until the dedicated fusion/attention migration", StageOwnershipStatus.CANONICAL),
