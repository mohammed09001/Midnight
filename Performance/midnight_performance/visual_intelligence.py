...
from .relationship_graph import GraphEdge, PerformanceGraph
...
def _node_record(node: VisualNode) -> Mapping[str, object]:
    return {"id": node.identity.canonical, "kind": node.entity_kind.value, "layer": node.layer, "label": node.label, "claim_kind": node.claim_kind.value, "provenance": node.provenance, "externally_referenced": node.externally_referenced, "gaps": node.gaps}
...
def build_performance_visual_map(graph: PerformanceGraph, *, project_context: str | None = None, node_labels: Mapping[Identity, str] | None = None, external_nodes: frozenset[Identity] = frozenset()) -> PerformanceVisualMap:
    """Adapt a rebuildable relationship graph without adding relationships or nodes."""
    node_labels = node_labels or {}
    graph_nodes = graph.nodes
    if not set(node_labels) <= graph_nodes or not external_nodes <= graph_nodes:
        raise ValueError("visual-map metadata may only describe graph nodes")
    nodes = tuple(VisualNode(node, node.kind, _layer(node.kind), node_labels.get(node, node.canonical), ClaimKind.DERIVED, externally_referenced=node in external_nodes) for node in sorted(graph_nodes, key=lambda item: item.canonical))
...
class PromptLineageVisualization:
    ...
    def as_records(self) -> tuple[Mapping[str, object], ...]:
        return ({"schema_version": self.schema_version, "revisions": tuple({"version_id": n.version_id, "parent_version_id": n.parent_version_id, "observed_at": n.observed_at.isoformat(), "requirement_counts": dict(n.requirement_counts), "claim_kind": n.claim_kind.value, "change_set_ids": n.change_set_ids, "verification_ids": n.verification_ids, "feedback_ids": n.feedback_ids, "runtime_references": tuple(f"{r.provider}:{r.kind}:{r.value}" for r in n.runtime_references), "gaps": n.gaps} for n in self.revisions), "edges": tuple({"parent_version_id": e.parent_version_id, "child_version_id": e.child_version_id, "added_constraints": e.added_constraints, "removed_constraints": e.removed_constraints, "added_acceptance": e.added_acceptance, "removed_acceptance": e.removed_acceptance, "added_verification": e.added_verification, "removed_verification": e.removed_verification, "unchanged_requirements": e.unchanged_requirements, "outcome_shift": repr(e.outcome_shift) if e.outcome_shift is not None else None, "method": e.method, "method_version": e.method_version, "uncertainty": e.uncertainty} for e in self.edges), "gaps": self.gaps},)


def build_prompt_lineage_visualization(revisions: tuple[PromptRevision, ...], links: tuple[PromptLineageLink, ...], *, change_sets: Mapping[str, tuple[str, ...]] | None = None, verification_ids: Mapping[str, tuple[str, ...]] | None = None, feedback_ids: Mapping[str, tuple[str, ...]] | None = None, runtime_references: Mapping[str, tuple[ExternalReference, ...]] | None = None) -> PromptLineageVisualization:
    ...
    known = set(ids)
    if any(key not in known for source in (change_sets, verification_ids, feedback_ids, runtime_references) for key in source): raise ValueError("lineage evidence must belong to represented revisions")
    ...
    return PromptLineageVisualization(VISUAL_PROJECTION_VERSION, tuple(nodes), edges, tuple(sorted(gaps)))
...
def as_query_projection(name: str, projection: PerformanceVisualMap | PromptLineageVisualization | ExperienceNeighborhoodVisualization):
    ...
    return QueryProjection(name, VISUAL_PROJECTION_VERSION, ClaimKind.DERIVED, projection.as_records(), (), "rebuildable Visual Intelligence projection; it is not canonical evidence, causal proof, or a recommendation")
